...
if ($search != '') {
    ...
    $where .= " AND (
        ...
        p.nama_pelanggan LIKE '%$searchEscaped%' OR 
        rp.alasan LIKE '%$searchEscaped%'
    ) ";
}

// Hitung total data tanpa filter (untuk DataTables)
$totalDataQuery = "SELECT COUNT(*) as total FROM retur_penjualan";
...
$totalFilteredQuery = "SELECT COUNT(*) as total FROM retur_penjualan rp
    LEFT JOIN pelanggans p ON rp.id_pelanggan = p.id_pelanggan
    $where";
...
$sql = "SELECT rp.id_retur_penjualan, rp.no_retur, rp.no_faktur, p.nama_pelanggan, 
